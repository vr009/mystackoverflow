...
class QuestionForm(forms.Form):
    ...
    text = forms.CharField(label='Текст', widget=forms.Textarea(
        attrs={'class': 'form-control', 'placeholder': 'Sense of question'}))
    tags = forms.CharField()

    def clean(self):
        cleaned_data = super().clean()
        if len(cleaned_data["title"]) > 30:
            raise forms.ValidationError('Слишком длинный заголовок вопроса')
...
class AnswerForm(forms.Form):
    text = forms.CharField(label='Your answer:', widget=forms.Textarea(
        attrs={'rows': 5, 'class': 'form-control', 'placeholder': 'Input here'}))
    ...
    def clean(self):
        cleaned_data = super().clean()
        if len(cleaned_data["text"]) > 255:
            raise forms.ValidationError('too long answer')
        return self.cleaned_data
